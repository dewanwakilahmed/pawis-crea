...
// Video Data
import videoData from '@/public/videos/videoData';
...
const getFilteredVideos = (filter: string, categories: string[]): Video[] => {
  if (filter === 'all') return videoData;
  if (filter === 'featured') {
    return categories
      .map((category) =>
        videoData.filter((video) => video.category === category).slice(0, 3)
      )
      .flat();
  }
  return videoData.filter((video) => video.category === filter);
};
...
const PhotoGallery: FC = () => {
  const categories = useMemo(
    () => Array.from(new Set(videoData.map((img) => img.category))),
    []
  );
